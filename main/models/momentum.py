...
class MomentumModel(db.Model):
    ...
    @classmethod
    def find_by_id(cls, _id) -> "MomentumModel":
    ...
    def get_distinct_days_by_month(cls, year: int, month: int) -> List:
        return db.session.query(func.distinct(cls.day)).filter_by(year=year, month=month).group_by(cls.day).order_by(cls.day.asc()).all()

    @classmethod
    def get_distinct_months_by_pair(cls, year: int, pair: str) -> List:
        return db.session.query(func.distinct(cls.month)).filter_by(year=year, pair=pair).group_by(cls.month).order_by(cls.month.asc()).all()
