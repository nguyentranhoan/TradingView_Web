...
document.querySelector("#swingTrading").onclick = resetALL;
...
function getPairByMonth()
{
    ...
    transaction_year = year_pattern.exec(document.querySelector('#chosenDate').value)[0];
    transaction_month = month_pattern.exec(document.querySelector('#chosenDate').value)[0];
    ...
    request.open('GET', "/"+strategy_name+"/"+transaction_year+"/"+transaction_month, true);
   ...
   if(by_disabled_pair == true)
      {
        ...
        if (check == true)
        {
            request.open('DELETE', '/'+strategy_name+'/'+transaction_year+'/'+transaction_month, true);
            request.onreadystatechange = function ()
            {
                if(request.readyState === 4 && request.status === 200)
                {
    ...
    else
    {
        ...
        if (check == true)
        {
            ...
            request.open('DELETE', '/'+strategy_name+'/'+transaction_year+'/'+transaction_month+'/'+transaction_pair, true);
            request.onreadystatechange = function ()
            {
                if(request.readyState === 4 && request.status === 200)
                {
